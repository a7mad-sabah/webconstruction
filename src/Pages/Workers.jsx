...
export default function Services() {
  ...
  const [jobFilter, setJobFilter] = useState("هەموو");
    ...
    const fetchWorkers = async () => {
      try {
        const res = await fetch(
          "http://localhost/goldenhand/my-project/backend/api/getWorkers.php",
        );
  ...
  return (
    <div dir="rtl" className="min-h-screen py-6 text-right">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative w-full md:w-64">
            <input
              ...
              className="w-full pr-10 pl-4 py-2 rounded-2xl bg-white/70 backdrop-blur-md border border-gray-200 shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none transition"
            />
      ...
      {!loading && filteredWorkers.length === 0 && (
        <p className="text-center text-gray-500">هیچ کارمەندێک نەدۆزرایەوە</p>
      )}

      <div className="w-[92%] max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-6 px-2">
        {filteredWorkers.map((worker, index) => (
          <GlowCard key={worker.id} worker={worker} index={index} />
        ))}
      </div>
    </div>
  );
...
function GlowCard({ worker, index }) {
...
جۆری کار: ${worker.job}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        "http://localhost/goldenhand/my-project/backend/api/addFeedback.php",
        {
          method: "POST",
          headers: {
      ...
      >
        ...
        <div className="relative z-10 bg-white/80 backdrop-blur-xl p-5 rounded-3xl flex flex-col items-center text-center">
          ...
          <p className="text-gray-500 text-xs mt-3 px-2">
            {worker.description || "بەردەست نییە"}
          </p>
